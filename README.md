...
- [Grove v0.3.2](https://github.com/xuechenf/grove/releases/tag/v0.3.2)
...
Version 0.4.0 moves Grove's structured control-plane state to a local SQLite database with transactional updates, WAL journaling, integrity checks, online backups, and an automatic one-time import from v0.3 YAML and Copilot JSONL state. Credential secrets and their encrypted vault, private keys, source trees, build artifacts, logs, and Terraform state remain outside SQLite.
