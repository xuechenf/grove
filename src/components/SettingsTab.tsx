...
export function SettingsTab({ vm, telemetry, applications, loading = false, onRefreshCloud, onTestConnection, onEditVm, onOpenInfrastructure, onRemoveVm }: SettingsTabProps) {
  ...
  async function removeRule(rule: CloudFirewallRule) {
    if (!machine || managedEnvironment || !window.confirm(`Remove ${formatFirewallRule(rule)} from ${rule.source}?`)) return
    setMutating(true)
    setError(undefined)
    try {
  ...
  return (
    <div className="space-y-4" data-testid="settings-tab">
      ...
      <section className="rounded border border-slate-200 bg-white" data-testid="security-groups-panel">
        ...
        {machine ? rulesLoading ? <p className="flex items-center gap-2 px-3 py-4 text-sm text-slate-500"><LoaderCircle className="h-4 w-4 animate-spin" />Loading security-group rules…</p> : <div className="divide-y divide-slate-100">{rules.length ? rules.map((rule) => <div key={rule.id} className="flex items-center gap-2 px-3 py-2 text-xs"><span className={cx('rounded px-1.5 py-0.5', rule.direction === 'ingress' ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-600')}>{rule.direction}</span><span className="font-mono text-slate-700">{formatFirewallRule(rule)}</span><span className="min-w-0 flex-1 truncate text-slate-500">{rule.source} · {rule.firewallName}</span>{rule.direction === 'ingress' && !managedEnvironment ? <button type="button" aria-label={`Remove ${formatFirewallRule(rule)} rule`} onClick={() => void removeRule(rule)} disabled={mutating} className="rounded p-1 text-rose-600 hover:bg-rose-50 disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" /></button> : null}</div>) : <p className="px-3 py-4 text-sm text-slate-500">No security-group rules returned.</p>}</div> : null}
      </section>
