...
import { cx, formatFirewallRule } from '../lib/format'
...
export function SettingsTab({ vm, telemetry, applications, loading = false, onRefreshCloud, onTestConnection, onEditVm, onOpenInfrastructure, onRemoveVm }: SettingsTabProps) {
  ...
  async function removeRule(rule: CloudFirewallRule) {
    if (!machine || managedEnvironment || !rule.removable || !window.confirm(`Remove ${formatFirewallRule(rule)} from ${rule.source}?`)) return
    setMutating(true)
    setError(undefined)
    try {
  ...
  return (
    <div className="space-y-4" data-testid="settings-tab">
      ...
      <section className="rounded border border-slate-200 bg-white">
        ...
        {machine ? <div className="grid gap-3 p-3 sm:grid-cols-2 xl:grid-cols-4"><Field label="Provider" value={machine.provider.toUpperCase()} /><Field label="Credential profile" value={machine.credentialProfileName} /><Field label="Region / zone" value={`${machine.location}${machine.zone ? ` / ${machine.zone}` : ''}`} /><Field label="Native machine ID" value={machine.nativeId ?? machine.id} mono /><Field label="Public IP" value={machine.publicIp ?? '—'} mono /><Field label="Private IP" value={machine.privateIp ?? '—'} mono /><Field label="VPC / virtual network" value={machine.vpcId ?? '—'} mono /><Field label="Subnet / VSwitch" value={machine.subnetId ?? '—'} mono /></div> : <div className="flex gap-3 p-4 text-sm text-slate-600"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-500" /><div><strong className="block text-slate-800">Host-managed VM</strong><p className="mt-1">No tested AWS, Azure, or Alibaba Cloud credential currently matches this VM. Grove continues to monitor it over SSH without presenting cloud-only controls.</p></div></div>}
      </section>
      ...
      <section className="rounded border border-slate-200 bg-white" data-testid="security-groups-panel">
        ...
        {machine ? rulesLoading ? <p className="flex items-center gap-2 px-3 py-4 text-sm text-slate-500"><LoaderCircle className="h-4 w-4 animate-spin" />Loading security-group rules…</p> : <div className="divide-y divide-slate-100">{rules.length ? rules.map((rule) => <div key={rule.id} className="flex items-center gap-2 px-3 py-2 text-xs"><span className={cx('rounded px-1.5 py-0.5', rule.direction === 'ingress' ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-600')}>{rule.direction}</span><span className="font-mono text-slate-700">{formatFirewallRule(rule)}</span><span className="min-w-0 flex-1 truncate text-slate-500">{rule.source} · {rule.firewallName}</span>{rule.removable && !managedEnvironment ? <button type="button" aria-label={`Remove ${formatFirewallRule(rule)} rule`} onClick={() => void removeRule(rule)} disabled={mutating} className="rounded p-1 text-rose-600 hover:bg-rose-50 disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" /></button> : <span className="rounded bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-500" title={rule.readOnlyReason}>Read only</span>}</div>) : <p className="px-3 py-4 text-sm text-slate-500">No security-group rules returned.</p>}</div> : null}
      </section>
